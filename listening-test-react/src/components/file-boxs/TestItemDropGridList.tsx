...
export const TestItemDropGridList = observer(function ({
  example,
  type = 'image',
  disableUpload,
  keepSlot,
  ...
  const { handleDragStart, handleDragOver, handleDropSwapFiles, handleDelete, handleAdd } = useFileBoxesFunc(
    example.medias,
    keepSlot,
  );
        ...
        <ImageListItem key={i}>
          ...
          {a ? (
            <ImageListItemBar
              subtitle={a?.filename}
              actionIcon={
                <IconButton className={classes.withe} onClick={() => handleDelete(i)}>
