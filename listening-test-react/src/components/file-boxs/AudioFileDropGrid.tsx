...
import { FileUploadDropBox, useFileBoxesFunc } from './FileUploadDropBox';

// reference means we can upload reference. keepPlace means the audio place will be kept after a deletion
export const AudioFileDropGrid = observer(function ({
  example,
  reference,
  keepPlace,
  allSame,
  ...
  const { handleDropSwapFiles, handleDragStart, draggingIndex, handleDragOver } = useFileBoxesFunc(
    example.medias,
    keepPlace,
  );
    ...
    <>
      ...
      {reference && (
        <Grid item xs={12} md={4}>
          <AudioFileUploadBox
            fileModel={example.mediaRef}
            onChange={fm => handleChange(fm, -1)}
      ...
      {example.medias.slice(0, allSame ? 1 : example.medias.length).map((a, i) => (
        <Grid item xs={12} md={4} key={i}>
          <AudioFileUploadBox
            fileModel={a}
            onChange={fm => handleChange(fm, i)}
            disabled={draggingIndex !== undefined}
            onDrop={() => handleDropSwapFiles(i)}
...
const AudioFileUploadBox = observer(
  ({
    onChange,
    fileModel,
    label,
    ...
    onChange: (fm: BasicFileModel) => void;
    fileModel?: BasicFileModel;
    label?: string;
    isTag?: boolean;
    onDragStart?: () => void;
    onDrop?: () => void;
    ...
    return (
      <div onDrop={onDrop} onDragStart={onDragStart} draggable={!!onDragStart} onDragOver={onDragOver}>
        <FileUploadDropBox onChange={onChange} fileType="audio" disabled={disabled}>
          {fileModel?.filename ? (
            ...
            <>
              <Typography className={classes.fileNameEllipsis}>
                {label ? label : 'Click to choose or Drop a file'}
              </Typography>
