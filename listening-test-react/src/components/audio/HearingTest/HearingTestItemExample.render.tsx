...
export const HearingTestItemExampleRender = observer(function (props: { value: AudioExampleModel; active?: boolean }) {
  ...
  const handleButtonClick = (audio: AudioFileModel, index: number) => {
    if (audio.isActive) {
      ...
      gos[index] = createOscillatorAndGain(
        audio.value ? +audio.value : audio.settings.initVolume,
        audio.settings.frequency,
      );
  ...
  return (
    <Grid container spacing={2}>
      ...
      {value.medias.map((v, i) => (
        <Grid item key={i} style={ratingAreaStyle}>
          <VolumeBar audio={v} gainNode={gos[i]?.gainNode} />
          <Button
            variant={v.isActive ? 'contained' : 'outlined'}
            color="primary"
            size="large"
            disableElevation
...
const VolumeBar = observer(function ({ audio, gainNode }: { audio: AudioFileModel; gainNode: GainNode }) {
  ...
  return (
    <Grid container spacing={1} direction="column" alignItems="center" className={classes.volumeBarContainer}>
      <Grid item>
        <Icon>volume_up</Icon>
      </Grid>
