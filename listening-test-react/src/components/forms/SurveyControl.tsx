      ...
      return (
        <>
          ...
          <TextField
            ...
            value="Subject will answer the question here..."
            disabled
          />
        </>
      );
...
const SurveyOptions = observer(function ({
  control,
  gotoQuestionItems,
  disableGoto,
}: {
    ...
    <Grid container spacing={2}>
      {control.options?.map((o, i) => (
        <Grid key={i} item xs={12} container spacing={1} alignItems="flex-end">
          ...
          {control.type === SurveyControlType.radio && gotoQuestionItems && (
            <Grid item>
              <Tooltip
                title={
                  disableGoto
                    ? 'You must enable "Show each question individually" in Global settings. Click to open the settings to check the option'
                    : ''
                }>
                <FormControl className={classes.selectWidth}>
                  <Select
                    inputProps={{ name: 'goto' }}
                    displayEmpty
      ...
      <Grid item xs={12} container spacing={1} alignItems="flex-end">
        ...
        <Grid item>{/*{!control.otherOption && <Button color="primary">Add the Other Option</Button>}*/}</Grid>
      </Grid>
    </Grid>
  );
});
