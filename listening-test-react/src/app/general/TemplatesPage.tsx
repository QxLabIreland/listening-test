                ...
                {templates.map((test) => (
                  <TableRow hover key={test._id.$oid}>
                    ...
                    <TableCell>
                      <Checkbox
                        color="primary"
                        checked={!!test.isTemplate}
                        onChange={() => handleIsTemplateChange(test)}
                      />
                    </TableCell>
                    <TableCell>
                      {currentUser._id.$oid === test.creator._id.$oid && (
                        <Tooltip title="Edit Template">
                          <IconButton size="medium" color="primary" onClick={() => handleTemplateEdit(test)}>
                            <Icon>edit</Icon>
                          </IconButton>
                        </Tooltip>
                      )}
  ...
  const handleTemplateEdit = (aTest: BasicTaskModel) => {
    ...
    if (aTest.isTemplate)
      openDialog(
        ...
        'Are you sure?',
        null,
        () => navigate(`/user/${testUrl}/${aTest._id.$oid}`),
      );
