...
it('render loading animation', () => {
  act(() => {
    render(
      <MemoryRouter initialEntries={['/user/ab-test/0']}>
...
it('renders detail data of a test', async () => {
  ...
  jest
    .spyOn(Axios, 'get')
    .mockImplementation(() => Promise.resolve({ data: fakeUser } as AxiosResponse<BasicTaskModel>));

  // Use the asynchronous version of act to apply resolved promises
  await act(async () => {
    render(
      <MemoryRouter initialEntries={['/user/ab-test/1']}>
        <TestDetailView testUrl="ab-test" ButtonGroup={AbAddItemButtonGroup} />
      </MemoryRouter>,
      container,
    );
